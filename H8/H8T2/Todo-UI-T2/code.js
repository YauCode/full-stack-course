...
function createTodoListItem(todo) {
    ...
    let span_attr_edit = document.createAttribute('class')
    span_attr_edit.value = 'edit'
    span_edit.setAttributeNode(span_attr_edit)
    let edit = document.createTextNode(' Edit ')
    span_edit.appendChild(edit)
    span_edit.onclick = function () { updateTodo(todo._id) }
...
//Function when Edit Span clicked
async function updateTodo(id) {

    let btn = document.getElementById("button")
    btn.style.backgroundColor = "yellow";
    btn.textContent = "Tallenna";
    btn.setAttribute("onClick", "tallennaTodo(\"" + id + "\")");
    ...
    let responseJson = await response.json()
    let newTodo = document.getElementById('newTodo')
    newTodo.value = responseJson.text

}

//Function when Tallenna button clicked
async function tallennaTodo(id) {
    let newTodo = document.getElementById('newTodo')
    const data = { 'text': newTodo.value }
    const response = await fetch('http://localhost:3000/todos/' + id, {
        method: 'PUT',
